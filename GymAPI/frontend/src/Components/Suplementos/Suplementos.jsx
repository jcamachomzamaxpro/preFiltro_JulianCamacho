...
export const Suplementos = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:3423/api/suplementos").then((response) => {
      setDatosAPI(response.data);
    });
  }, []);

  const getDatos = () => {
    axios.get(`http://localhost:3423/api/suplementos`).then((dataObtained) => {
      setDatosAPI(dataObtained.data);
    });
  };

  const onDelete = (id) => {
    axios.delete(`http://localhost:3423/api/suplementos/${id}`).then(() => {
      getDatos();
    });
  };
    ...
    <div className="global-container">
      ...
      <div className="renderDatos">
        {datosAPI.map((datos) => {
          ...
          return (
            <div class="card">
              <img src={imagen} className="cardImg" />
